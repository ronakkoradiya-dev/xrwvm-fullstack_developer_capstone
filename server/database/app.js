...
app.get('/get_cars', async (req, res) => {
    try {
        const cars_list = await Car.find();
        // Format them so React receives CarMake and CarModel keys as expected by your frontend
        const formattedCars = cars_list.map(car => ({
            CarMake: car.make,
            CarModel: car.model
        }));
        res.json({ CarModels: formattedCars });
    } catch (error) {
        res.status(500).json({ error: 'Error fetching cars from inventory' });
    }
});